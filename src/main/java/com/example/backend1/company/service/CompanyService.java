...
import com.example.backend1.company.domain.CompanySource;
...
public class CompanyService {
    ...
    public List<CompanyDtos.NearbyCompanyResponse> findNearby(double userLat, double userLon, String keyword) {
        ...
        for (Company c : partners) {
            ...
            // ADMIN 소스만 제휴 업체, KAKAO 소스(리뷰 작성 시 lazy 생성)는 외부 업체
            boolean isPartner = c.getSource() == CompanySource.ADMIN;
            combined.add(new CompanyDtos.NearbyCompanyResponse(
                    c.getId(), c.getName(), c.getPhone(), c.getAddressLine(),
                    lat, lng, distanceKm, isPartner,
                    c.getKakaoPlaceId(), null, 0   // 리뷰 통계는 후처리에서 채운다
            ));
